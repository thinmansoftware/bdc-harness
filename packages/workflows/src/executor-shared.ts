...
/**
 * Build a regex that matches `$NAME` only as a whole identifier: the next
 * character must not be [A-Za-z0-9_]. Without the negative lookahead a longer
 * shell variable that merely begins with a workflow variable name (e.g.
 * $BASE_BRANCH_OVERRIDE in lane bash nodes) would be corrupted at substitution
 * time -- "$BASE_BRANCH_OVERRIDE" became the literal "dev_OVERRIDE"
 * (bdc-harness #877).
 */
