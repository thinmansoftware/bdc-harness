...
export function substituteWorkflowVariables(
  ...
  // Bounded by a negative lookahead so $BASE_BRANCH_OVERRIDE (a longer identifier
  // sharing the prefix) does NOT trigger the guard -- only an exact $BASE_BRANCH
  // reference does. bdc-harness#877.
  ...
  // Substitute basic variables. Every $NAME pattern is bounded by a negative
  // lookahead (?![A-Za-z0-9_]) so a longer identifier sharing the same prefix
  // (e.g. $BASE_BRANCH_OVERRIDE, $BASE_BRANCH_PR) is left intact. bdc-harness#877.
  // ${run.id} is brace-delimited and already exact -- left unchanged.
  let result = prompt
