...
export function substituteWorkflowVariables(
  ...
  // Bounded so that a longer name such as $BASE_BRANCH_OVERRIDE does not trip the guard.
  ...
  // Substitute basic variables. Each $NAME match is bounded with a negative lookahead
  // so that a longer identifier that merely starts with the variable name (for example
  // $BASE_BRANCH_OVERRIDE or $BASE_BRANCH_PR) is left untouched for the shell to handle.
  let result = prompt
