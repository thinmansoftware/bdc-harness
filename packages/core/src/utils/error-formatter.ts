...
export function classifyAndFormatError(error: Error): string {
  ...
  if (
    ...
    return '! Claude authentication expired. Run `/login` inside Claude Code or `claude logout && claude login` in your terminal.';
  }
  ...
  if (message.startsWith('Claude Code auth error:')) {
    return '! Claude authentication error. Run `/login` inside Claude Code or check your API key configuration.';
  }
  ...
  if (
    ...
    return '! Codex authentication error. Run `codex login` in your terminal to re-authenticate.';
  }
  ...
  if (
    ...
    return '! AI service authentication error. Please check your API key or credentials.';
  }

  // Network errors - timeout
  if (message.includes('timeout') || message.includes('ETIMEDOUT')) {
    return '! Request timed out. The AI service may be slow. Try again or use /reset.';
  }
  ...
  if (message.includes('session') || message.includes('Session')) {
    return '! Session error. Use /reset to start a fresh session.';
  }
  ...
  if (message.includes('Codex query failed:')) {
    ...
    return `! AI error: ${innerMessage}. Try /reset if issue persists.`;
  }
  ...
  if (
    ...
    return `! Error: ${message}. Try /reset if issue persists.`;
  }

  // True generic fallback for unknown/sensitive errors
  return '! An unexpected error occurred. Try /reset to start a fresh session.';
}
