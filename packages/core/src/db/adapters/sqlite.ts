...
export class SqliteAdapter implements IDatabase {
  ...
  private createSchema(): void {
    this.db.run(`
      ...
      -- Recovery execution claims (migration 038): fenced repair/refire ownership.
      CREATE TABLE IF NOT EXISTS remote_agent_recovery_execution_claims (
        ...
        CONSTRAINT recovery_execution_claims_attempt_state CHECK (
          ...
          OR (effect_attempt_state IN ('armed', 'completed', 'released')
      ...
      CREATE TRIGGER IF NOT EXISTS trg_recovery_execution_claim_events_no_update
        BEFORE UPDATE ON remote_agent_recovery_execution_claim_events
        BEGIN
          SELECT RAISE(ABORT, 'remote_agent_recovery_execution_claim_events is append-only');
        END;

      CREATE TRIGGER IF NOT EXISTS trg_recovery_execution_claim_events_no_delete
