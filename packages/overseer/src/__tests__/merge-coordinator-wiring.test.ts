...
/**
 * Exact heartbeat object for a watch tick whose deps have no listOpenPullRequests.
 * Discovery reports itself unavailable (`no_list_dep`) and contributes zeros.
 */
function heartbeatWithoutDiscovery(counts: {
  evaluated: number;
  total: number;
  eligible: number;
  ...
  return {
    ...
    total: counts.total,
    runsEvaluated: counts.evaluated,
    runsTotal: counts.total,
    prsEvaluated: 0,
    prCandidates: 0,
    prDiscoveryUnavailable: true,
    prDiscoveryUnavailableReason: 'no_list_dep',
    prsFallbackDecision: 0,
    prsTotalOpen: 0,
    prsWindowTruncated: false,
    exclusionsByReason: {},
...
describe('merge coordinator wiring (WO-HARNESS-MERGE-MANAGER-WIRING-LAND-01)', () => {
  ...
  test('Test 2: red / conflicting / no-verdict PRs are skipped loudly and never merged', async () => {
    ...
    expect(heartbeat?.obj).toEqual(
      heartbeatWithoutDiscovery({ evaluated: 3, total: 3, eligible: 0 })
    );
  ...
  test('Test 3: an empty watch cycle still emits a merge-coordinator heartbeat with zeroed counts', async () => {
    ...
    expect(heartbeat?.obj).toEqual(
      heartbeatWithoutDiscovery({ evaluated: 0, total: 0, eligible: 0 })
    );
  });
  ...
  test('Test 4: heartbeat counts a merge-ready run as eligible and the real gate dispatches it', async () => {
    ...
    expect(heartbeat).toBeDefined();
    expect(heartbeat?.obj).toEqual(
      heartbeatWithoutDiscovery({ evaluated: 2, total: 3, eligible: 1 })
    );
