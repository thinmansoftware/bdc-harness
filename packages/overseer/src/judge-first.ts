 ...
 * (db layer). The persistent claim table caps total calls at (runs x retries),
 * which is the real backstop; the former in-memory daily budget circuit was
 * removed (WO-HARNESS-OVERSEER-UNBURY-GATES-01) because it silently killed 60%
 * of verdicts with no operator signal -- a buried gate, not a control.
 */
