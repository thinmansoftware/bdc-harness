 ...
 *
 *   1. JSON file at <archonHome>/runs/<runId>/escalation.json (the operator card),
 *      containing the full structured context (errorClass, nodeId, validatorOutput,
 ...
 *
 * WO context (woId and related fields) is supplied by the caller (the decision
 * layer) and, where richer detail is needed, sourced from GitHub (issue body /
 * labels) -- the system of record for WORK since 2026-07-01. This module does NOT
 * query the legacy WO tracker database: it holds no truth for any post-cutover
 * WO, so that lookup was structurally guaranteed to fail every time and has been
 * removed.
 *
 * Design notes:
 *   - Fail-soft on individual side effects: if one signal fails, the others STILL
 *     fire. The contract is "operator gets at least one signal".
 ...
 * populated by this module rather than the decision layer.
 */
 ...
 * webhook) fails, the others are still attempted. Errors are captured and
 * surfaced via the return value rather than thrown -- the caller
