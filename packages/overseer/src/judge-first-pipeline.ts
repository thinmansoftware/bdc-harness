  ...
  if (record.recovery?.plan === 'operator_card') {
    ...
    await escalateWithEvidence(
      ...
      events,
      {
        ...
        blocker: `spec Tests-line defect: run-stop-tests found no runnable spec-declared test command (TESTS_SOURCE=${source}, TESTS_STATUS=${status}); the code is not established as the failure; a spec amendment is required (WO-HARNESS-OVERSEER-REPAIR-IN-PLACE-01)`,
      },
      options.escalate
    );
    return;
