...
- `@openai/codex-sdk` bumped from `0.125.0` to `0.144.5`; default Codex workflow pins and the provider fallback moved from `gpt-5.5` to the live-verified `gpt-5.6-sol` model. SDK declaration diff shows no breaking TypeScript API changes consumed by Archon; `McpToolCallItem.result` only gained optional `_meta`, and `TurnCompletedEvent` still does not expose a served model. Live probe evidence: `bunx --bun @openai/codex@0.144.5 debug models` lists `gpt-5.6-sol` with `supported_in_api: true`, `priority: 1`; short inference returned `OK` with 5 output tokens. WO: WO-HARNESS-CODEX-SDK-BUMP-AND-MODEL-PINS-01.
- `@anthropic-ai/claude-agent-sdk` bumped from `0.2.121` to `0.3.197`. The intent is that the `sonnet` alias resolves to `claude-sonnet-5` (Sonnet 5) instead of `claude-sonnet-4-6` at runtime. The alias-to-model-id table is served by the Claude binary at initialization and is not statically inspectable from the package files; the `sdk.d.ts` type documentation uses `'sonnet' -> 'claude-sonnet-5'` as a field-description example (line 1172: "Canonical wire model id this row's value resolves to (e.g. 'sonnet' -> 'claude-sonnet-5')"), which is illustrative but not a declaration of the runtime binding. Pre-bump served model evidence: `served_model_id=claude-sonnet-4-6` captured in `remote_agent_workflow_events` (workflow_run_id ad34af189e56ff3659b178b8893e9360). Post-bump empirical verification of `served_model_id` via sqlite3 query requires a container rebuild with the new image (cold deploy requires John's PROCEED -- see stop condition 3 in WO-HARNESS-SONNET-5-MODEL-ALIAS-UPGRADE-01).
- `@anthropic-ai/sdk` bumped transitively from `0.81.0` to `0.93.0`. The `0.93.0` floor is required to satisfy the `claude-agent-sdk@0.3.197` peer dependency declaration (`"peerDependencies": { "@anthropic-ai/sdk": ">=0.93.0" }`). Previously `@anthropic-ai/sdk@0.90.0` was deduped to top-level from `@mariozechner/pi-ai`'s nested dependency when the `claude-agent-sdk` bump removed its former direct `^0.81.0` dep; `0.90.0` does not satisfy the new `>=0.93.0` peer requirement, so `@anthropic-ai/sdk` is now an explicit direct dependency of `@archon/providers` at `^0.93.0`. Paths potentially affected by the transitive bump: `@mariozechner/pi-ai` (previously nested its own `@anthropic-ai/sdk@0.90.0`; the dep graph now uses `0.93.0` for all consumers). WO: WO-HARNESS-SONNET-5-MODEL-ALIAS-UPGRADE-01.
