...
assert_eq "rc 1" "1" "${R%%|*}"
assert_contains "error names the declared count and status" "9 grep stop condition(s) were declared but grep_status=all_dropped" "$R"
...
echo "--- Test 7b: declared greps with incomplete execution -> error ---"
R="$(run_check "$GOOD" PROCEED CODE passed 5 incomplete)"
