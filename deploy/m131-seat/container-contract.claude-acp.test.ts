/** Static, hermetic M-131 Phase B Claude-ACP (ACP/MCP leg) seat packaging contract. */
...
const composeRaw = await Bun.file(join(dir, 'docker-compose.claude-acp.example.yml')).text();
const seatConfig = (await Bun.file(join(dir, 'config.seat.claude-acp.example.json')).json()) as {
...
const service = compose.services['bdc-seat-claude-acp'] as ComposeService;

describe('m131 Claude-ACP seat container contract', () => {
  ...
  test('mounts one read-only ingress and isolated writable profile/state volumes', () => {
    ...
    expect(service.volumes).toContain('m131-claude-acp-vendor-profile:/home/seat/.claude');
    expect(service.volumes).toContain('m131-claude-acp-state:/var/lib/bdc-seat');
    expect(compose.volumes).toHaveProperty('m131-claude-acp-vendor-profile');
    expect(compose.volumes).toHaveProperty('m131-claude-acp-state');
    expect(Object.keys(compose.services)).toEqual(['bdc-seat-claude-acp']);
  });

  test('advertises only the Claude ACP transport with concurrency one', () => {
    expect(seatConfig.seat.model_family).toBe('claude');
    expect(seatConfig.seat.provider_allowlist).toEqual(['claude-acp']);
    expect(seatConfig.agents ?? {}).toEqual({});
    expect(seatConfig.max_concurrency).toEqual({ 'claude-acp': 1 });
  ...
  test('does not share a home/profile/state with the sibling plain-CLI claude seat', () => {
    expect(seatConfig.seat.vendor_profile_dir).toBe('/home/seat/.claude');
    expect(compose.volumes).not.toHaveProperty('m131-claude-vendor-profile');
    expect(compose.volumes).not.toHaveProperty('m131-claude-state');
  });

  test('no literal secret in Claude-ACP seat packaging', () => {
